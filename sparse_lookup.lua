-- directly copy the code from LookupTable and made the changes
local SparseUpdateLookupTable, parent = torch.class('nn.SparseUpdateLookupTable', 'nn.Module')

function SparseUpdateLookupTable:__init(nIndex, ...)
   parent.__init(self)
   local arg = {...}
   ...
   -- lpk: gSquareOverTime used in the adagrad
   self.gSquareOverTime = torch.Tensor(self.size):zero()
...
function SparseUpdateLookupTable:accUpdateOnly()
   self.accUpdate = true
   self.gradWeight = nil
end

function SparseUpdateLookupTable:reset(stdv)
   stdv = stdv or 1
   if nn.oldSeed then
...
function SparseUpdateLookupTable:updateOutput(input)
...
function SparseUpdateLookupTable:zeroGradParameters()
...
function SparseUpdateLookupTable:accUpdateGradParameters(input, gradOutput, lr)
   if input:dim() == 1 then
      for i=1,input:size(1) do
         local k = input[i]
...
function SparseUpdateLookupTable:updateParameters(learningRate)
   assert(not self.accUpdate, "use accUpdateGradParameters instead")
   for k,nBackward in pairs(self.inputs) do
      local kscale = self:scaleUpdateByKey(k)
      if self.gradWeight:select(1, k)[1] ~= 0 then
         -- print ("***")
         local sqGrad_k = torch.pow(self.gradWeight:select(1,k),2)
         -- print (self.gSquareOverTime:select(1,k))
         -- print (sqGrad_k)
         self.gSquareOverTime:select(1,k):add(sqGrad_k)
         -- print (self.gSquareOverTime:select(1,k))

         -- print ((torch.sqrt(self.gSquareOverTime:select(1,k)))[1])
         local inv_sqrt_gSquareOverTime_k = 1.0 / ((torch.sqrt(self.gSquareOverTime:select(1,k)))[1])
         -- print (inv_sqrt_gSquareOverTime_k)
         -- print ("original")
         -- print (self.weight:select(1, k))

         self.weight:select(1, k):add(-learningRate*kscale*inv_sqrt_gSquareOverTime_k, self.gradWeight:select(1, k))

         -- print (-learningRate*kscale*inv_sqrt_gSquareOverTime_k)
         -- print (self.gradWeight:select(1, k))
         -- print ("after")
         -- print (self.weight:select(1, k))
         -- print ("done")
...
function SparseUpdateLookupTable:type(type)
   self._indices = nil
   self._inputView = nil
...
function SparseUpdateLookupTable:scaleUpdateByKey(inputKey)
   -- default is to perform no key-based scalling
   return 1
end

-- we do not need to accumulate parameters when sharing
SparseUpdateLookupTable.sharedAccUpdateGradParameters = SparseUpdateLookupTable.accUpdateGradParameters